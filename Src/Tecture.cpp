...
*/
	glm::vec4 ImageData::GetColor(int x, int y)const
	{
		...
		const uint16_t c = p[0] + p[1] * 0x100;//二つのバイトを結合
		if (format == GL_BGRA) {
			//16ビットのデータから各色を取り出す
...
*/
	...
	*/
	GLuint LoadImage2D(const char*path) {
		...
		return CreateImage2D(imageData.width, imageData.height,
			imageData.data.data(), imageData.format, imageData.type);
	}

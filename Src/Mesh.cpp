	...
	int* pStride = nullptr)
{
	...
	if (!accessor["count"].is_null()) {
		int unitByteSize;
		switch (accessor["conpornentaaType"].int_value()) {
		case GL_BYTE:
...
*/
	...
	*/
	...
	@return データを追加した位置
	        プリミティブ用indicesパラメータとして使うことができる
	*/
	...
	@param type     インデックスデータの型
	                (GL_UNSIGNED_BITE, GL_UNSIGNED_SHORT, GL_UNSIGNED_INTのいずれか)
	...
	*/
	FilePtr Buffer::GetFile(const char* name)const
	{
		...
		if (itr == files.end()) {
			std::cerr << "[警告]" << __func__ << ":" << name << "という名前のメッシュは追加されてません\n";

...
//地形テクスチャを合成
	...
	fragColor.rgb = texture(texColorArray[1], uv*10.0).rgb * baseRatio;
	fragColor.rgb += texture(texColorArray[2], uv*10.0).rgb * ratio.r;
	fragColor.rgb += texture(texColorArray[3], uv * 30.0).rgb * ratio.g;
	fragColor.a = 1.0;
